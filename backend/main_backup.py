...
def get_rooms(q:str=""):
    ...
    for n in GRAPH["nodes"]:
        ...
        seen.add(n["id"])
        rt=type_en(n.get("type",""))
        if q and q.lower() not in n["id"].lower() and q.lower() not in rt.lower(): continue
        results.append({"id":n["id"],"type":rt,"building":n.get("building",""),"floor":n.get("floor_num","")})
    return results[:100]
...
_C3ADJ = _defaultdict(list)
for _key, _data in C3["floors"].items():
    _bldg, _fl = _key.split("_"); _fl = int(_fl)
    for _e in _data["edges"]:
        _a = (_bldg, _fl, _e["from"]); _b = (_bldg, _fl, _e["to"])
        _C3ADJ[_a].append((_b, _e["dist"]))
        _C3ADJ[_b].append((_a, _e["dist"]))
for _sl in C3["stair_links"]:
    _a=tuple(_sl["from"]); _b=tuple(_sl["to"])
    _C3ADJ[_a].append((_b, _sl["cost"]))
    _C3ADJ[_b].append((_a, _sl["cost"]))
for _cl in C3.get("cross_building_links", []):
    _a=tuple(_cl["from"]); _b=tuple(_cl["to"])
    _C3ADJ[_a].append((_b, _cl["cost"]))
    _C3ADJ[_b].append((_a, _cl["cost"]))

def _get_c3_node(room_id):
    n = NODES.get(room_id)
    if not n: return None
    key = f"{n.get('building','21')}_{n.get('floor','')}"
    idx = C3["floors"].get(key, {}).get("room_to_cpt", {}).get(room_id)
    if idx is None: return None
    return (n.get("building","21"), int(n.get("floor", 0)), int(idx))

@app.get("/api/route4")
def get_route4(from_room: str, to_room: str):
    """Rasterized-corridor routing — stays inside walkable space."""
    if from_room not in NODES: raise HTTPException(404, f"Room '{from_room}' not found")
    if to_room   not in NODES: raise HTTPException(404, f"Room '{to_room}' not found")
    if from_room == to_room:
        return {"corridor_path":[],"steps":[{"text":"You are already here!","type":"arrived","room":from_room}],"num_corridors":0}

    start = _get_c3_node(from_room)
    end   = _get_c3_node(to_room)
    if not start: raise HTTPException(404, f"No corridor point for '{from_room}'")
    if not end:   raise HTTPException(404, f"No corridor point for '{to_room}'")
    ...
    while pq:
        ...
        if d>dist.get(u,float("inf")): continue
        if u==end: break
        for v,w in _C3ADJ.get(u,[]):
            nd=d+w
            if nd<dist.get(v,float("inf")):
                dist[v]=nd; prev[v]=u; heapq.heappush(pq,(nd,v))

    if end not in dist: raise HTTPException(404,"No corridor route found")
    ...
    prev_f2=fr_n.get("floor")
    for c in corridor_coords:
        if c["floor"]!=prev_f2:
            direction="up" if c["floor"]>prev_f2 else "down"
            steps.append({"text":f"Take stairs {direction} to Floor {c['floor']}","type":"stairs","room":None,"floor":c["floor"]})
            prev_f2=c["floor"]
    steps.append({"text":f"You have arrived! Room {to_room}"+(f" ({type_en(tr_n.get('type',''))})" if tr_n.get('type') else ""),"type":"arrived","room":to_room})

    # Smooth path: remove collinear intermediate points
    # but limit max jump distance to avoid cutting through walls visually
    def smooth(coords, tol=80, max_jump=450):
        if len(coords)<=2: return coords
        result=[coords[0]]
        for i in range(1,len(coords)-1):
            p=result[-1]; c=coords[i]; n=coords[i+1]
            if p["floor"]!=c["floor"] or c["floor"]!=n["floor"]:
                result.append(c); continue
            # Don't skip if the jump from prev to next is too large
            import math as _m
            ...
            length=max(1,(dx1**2+dy1**2)**0.5)
            if cross/length>tol: result.append(c)
        result.append(coords[-1])
        return result

    # corridor_coords = smooth(corridor_coords)
